...
function LedgerEntry({ actionLabel, entry, identityLabel, isActionable, onDelete, onEdit, unavailableActionReason }: Pick<MonthlyLedgerProps, "actionLabel" | "identityLabel" | "isActionable" | "onDelete" | "onEdit" | "unavailableActionReason"> & { entry: LedgerViewEntry }) {
  ...
  return <article className="monthly-ledger-entry" data-entry-key={entry.entryKey}>
    <div className="monthly-ledger-row"><time dateTime={entry.occurredAt}>{time(entry.occurredAt)}</time><div><strong>{entryLabel}</strong><span>{directionLabels[entry.direction]}</span>{identity ? <span>{identity}</span> : null}<span>{entry.metadata.description ?? entryLabel}</span></div><strong className="monthly-ledger-amount">{money(entry.amount)}</strong></div>

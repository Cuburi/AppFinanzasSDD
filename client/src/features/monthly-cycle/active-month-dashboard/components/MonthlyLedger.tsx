...
function previewDays(days: LedgerDay[], limit: number): LedgerDay[] {
  let remaining = limit;
  const result: LedgerDay[] = [];

  for (const ledgerDay of days) {
    if (remaining <= 0) break;
    const items = ledgerDay.items.slice(0, remaining);
    if (items.length > 0) result.push({ ...ledgerDay, items });
...
function LedgerEntry({ actionLabel, entry, identityLabel, isActionable, onDelete, onEdit, unavailableActionReason }: Pick<MonthlyLedgerProps, "actionLabel" | "identityLabel" | "isActionable" | "onDelete" | "onEdit" | "unavailableActionReason"> & { entry: LedgerViewEntry }) {
  ...
  const isPocketDeposit = entry.eventType === "POCKET_DEPOSIT_FROM_SUBCATEGORY" || entry.eventType === "POCKET_DEPOSIT_FROM_AVAILABLE";
  const movementIcon = entry.direction === "INFLOW" ? "+" : isPocketDeposit ? "↓" : "−";
  const amountTone = entry.direction === "INFLOW" ? "positive" : entry.direction === "OUTFLOW" && !isPocketDeposit ? "negative" : undefined;
  const signedAmount = `${entry.direction === "INFLOW" ? "+ " : entry.direction === "OUTFLOW" ? "− " : ""}${money(Math.abs(entry.amount))}`;
  const previewTitle = entry.metadata.description ?? actionName.replace(/^(ingreso|gasto)\s+/i, "") ?? entryLabel;

  return <article className="monthly-ledger-entry" data-entry-key={entry.entryKey}>
    <details><summary className="monthly-ledger-disclosure"><div className="monthly-ledger-row"><span aria-hidden="true" className="monthly-ledger-icon">{movementIcon}</span><div className="monthly-ledger-summary"><strong>{previewTitle}</strong><small>{entryLabel} · <time dateTime={entry.occurredAt}>{time(entry.occurredAt)}</time></small><span className="sr-only">{directionLabels[entry.direction]}</span>{identity ? <span className="sr-only">{identity}</span> : null}</div><strong className={`monthly-ledger-amount${amountTone ? ` ${amountTone}` : ""}`}>{signedAmount}</strong>{entry.direction !== "TRANSFER" ? <span className="sr-only">{money(entry.amount)}</span> : null}</div><span className="monthly-ledger-disclosure-show">Ver detalles de {actionName}</span><span className="monthly-ledger-disclosure-hide">Ocultar detalles de {actionName}</span></summary><dl className="monthly-ledger-details"><div><dt>Origen</dt><dd>Origen: {entityLabels[entry.source.kind]}</dd></div><div><dt>Destino</dt><dd>Destino: {entityLabels[entry.destination.kind]}</dd></div><div><dt>Descripción</dt><dd>Descripción: {entry.metadata.description ?? entryLabel}</dd></div><div><dt>Medio de pago</dt><dd>Medio de pago: {entry.metadata.paymentMethod === "NON_CASH" ? "No efectivo" : entry.metadata.paymentMethod === "CASH" ? "Efectivo" : "Sin registrar"}</dd></div><div><dt>Efectos</dt><dd>Disponible del mes: {money(entry.balanceEffects.availableMoney)}</dd><dd>Saldo en efectivo: {money(entry.balanceEffects.cashBalance)}</dd><dd>Disponible de subcategoría: {money(entry.balanceEffects.subcategoryAvailable)}</dd><dd>Saldo del bolsillo: {money(entry.balanceEffects.pocketBalance)}</dd></div></dl></details>
...
export function MonthlyLedger({ actionLabel, days, identityLabel, isActionable, onDelete, onEdit, onRetry, status, unavailableActionReason }: MonthlyLedgerProps) {
  ...
  const displayedDays = managerOpen ? days : previewDays(days, 5);

  return <Card aria-label="Movimientos del mes" className={`monthly-ledger${managerOpen ? " monthly-ledger-expanded" : ""}`} role="region"><SectionHeader description="Actividad completa del mes en el orden registrado." title="Movimientos del mes" />

...
function LedgerEntry({ actionLabel, entry, isActionable, onDelete, onEdit, unavailableActionReason }: Pick<MonthlyLedgerProps, "actionLabel" | "isActionable" | "onDelete" | "onEdit" | "unavailableActionReason"> & { entry: LedgerViewEntry }) {
...
export function MonthlyLedger({ actionLabel, days, isActionable, onDelete, onEdit, onRetry, status, unavailableActionReason }: MonthlyLedgerProps) {
  return <Card aria-label="Movimientos del mes" className="monthly-ledger" role="region"><SectionHeader description="Actividad completa del mes en el orden registrado." title="Movimientos del mes" />
    ...
    {days.map((ledgerDay) => <section aria-labelledby={`ledger-day-${ledgerDay.key}`} className="monthly-ledger-day" key={ledgerDay.key}><h3 id={`ledger-day-${ledgerDay.key}`}>{day(ledgerDay.key)}</h3>{ledgerDay.items.map((item, index) => item.kind === "entry" ? <div data-ledger-item={item.entry.entryKey} key={item.entry.entryKey}><LedgerEntry actionLabel={actionLabel} entry={item.entry} isActionable={isActionable} onDelete={onDelete} onEdit={onEdit} unavailableActionReason={unavailableActionReason} /></div> : <SystemRun actionLabel={actionLabel} entries={item.entries} isActionable={isActionable} key={`${item.entries[0].entryKey}-${index}`} onDelete={onDelete} onEdit={onEdit} unavailableActionReason={unavailableActionReason} />)}</section>)}
  </Card>;
}

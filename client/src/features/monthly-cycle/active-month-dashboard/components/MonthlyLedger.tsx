...
const readOnlyReason = (entry: LedgerViewEntry) => {
...
function LedgerEntry({ entry, onDelete, onEdit }: Pick<MonthlyLedgerProps, "onDelete" | "onEdit"> & { entry: LedgerViewEntry }) {
  const hasAvailableAction = !entry.isReadOnly && (onEdit || onDelete);
  ...
  return <article className="monthly-ledger-entry" data-entry-key={entry.entryKey}>
    <div className="monthly-ledger-row"><time dateTime={entry.occurredAt}>{time(entry.occurredAt)}</time><div><strong>{entryLabel}</strong><span>{directionLabels[entry.direction]}</span><span>{entry.metadata.description ?? entryLabel}</span></div><strong className="monthly-ledger-amount">{money(entry.amount)}</strong></div>
    <dl className="monthly-ledger-details"><div><dt>Origen</dt><dd>Origen: {entityLabels[entry.source.kind]}</dd></div><div><dt>Destino</dt><dd>Destino: {entityLabels[entry.destination.kind]}</dd></div><div><dt>Medio de pago</dt><dd>Medio de pago: {entry.metadata.paymentMethod === "NON_CASH" ? "No efectivo" : entry.metadata.paymentMethod === "CASH" ? "Efectivo" : "Sin registrar"}</dd></div><div><dt>Efectos</dt><dd>Disponible del mes: {money(entry.balanceEffects.availableMoney)}</dd><dd>Saldo en efectivo: {money(entry.balanceEffects.cashBalance)}</dd><dd>Disponible de subcategoría: {money(entry.balanceEffects.subcategoryAvailable)}</dd><dd>Saldo del bolsillo: {money(entry.balanceEffects.pocketBalance)}</dd></div></dl>
    {hasAvailableAction ? <div className="monthly-ledger-actions">{onEdit ? <Button onClick={() => onEdit(entry)} type="button" variant="secondary">Editar {entry.metadata.description ?? entryLabel}</Button> : null}{onDelete ? <Button onClick={() => onDelete(entry)} type="button" variant="tertiary">Eliminar {entry.metadata.description ?? entryLabel}</Button> : null}</div> : <p className="monthly-ledger-read-only">{readOnlyReason(entry)}</p>}
  </article>;
}

function SystemRun({ entries, onDelete, onEdit }: Pick<MonthlyLedgerProps, "onDelete" | "onEdit"> & { entries: LedgerViewEntry[] }) {
  const [open, setOpen] = useState(false); const id = useId(); const count = entries.length;
  return <section className="monthly-ledger-system-run" data-ledger-item={entries[0].entryKey}><button aria-controls={id} aria-expanded={open} className="monthly-ledger-disclosure" onClick={() => setOpen(!open)} type="button">{count} {count === 1 ? "movimiento automático" : "movimientos automáticos"}</button>{open ? <div id={id}>{entries.map((entry) => <LedgerEntry entry={entry} key={entry.entryKey} onDelete={onDelete} onEdit={onEdit} />)}</div> : null}</section>;
}

export function MonthlyLedger({ days, onDelete, onEdit, onRetry, status }: MonthlyLedgerProps) {
  return <Card aria-label="Movimientos del mes" className="monthly-ledger" role="region"><SectionHeader description="Actividad completa del mes en el orden registrado." title="Movimientos del mes" />
    ...
    {days.map((ledgerDay) => <section aria-labelledby={`ledger-day-${ledgerDay.key}`} className="monthly-ledger-day" key={ledgerDay.key}><h3 id={`ledger-day-${ledgerDay.key}`}>{day(ledgerDay.key)}</h3>{ledgerDay.items.map((item, index) => item.kind === "entry" ? <div data-ledger-item={item.entry.entryKey} key={item.entry.entryKey}><LedgerEntry entry={item.entry} onDelete={onDelete} onEdit={onEdit} /></div> : <SystemRun entries={item.entries} key={`${item.entries[0].entryKey}-${index}`} onDelete={onDelete} onEdit={onEdit} />)}</section>)}
  </Card>;
}

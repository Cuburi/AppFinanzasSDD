...
describe("ReportsPage", () => {
  ...
  it("fetches and renders the basic report for the active month only", async () => {
    ...
    expect(screen.getByText("Monthly income: $3200.00")).toBeInTheDocument();
    expect(screen.getByText("Available money: $850.00")).toBeInTheDocument();
    expect(screen.getByText("Cash balance: $120.00")).toBeInTheDocument();
  ...
  it("shows empty report states when the active month has no report rows", async () => {
    ...
    expect(await screen.findByText("Monthly income: $0.00")).toBeInTheDocument();

...
const requiredSnippets = [
  ...
  'The dev reset may recreate a missing disposable dev target or recover an unmarked dev target only after the Compose service, volume, labels, host port, database, and sole ownership match the dev policy.',
  'Personal reset stays stricter: it never bootstraps a missing target, but an existing unmarked personal target may reset only after exact confirmation and full local identity proof: Compose service, project label, host port, database, expected writable mount, expected volume metadata labels, and sole ownership.',
  'Mismatched markers, dev-vs-personal markers, wrong service, wrong volume, wrong port, wrong database, remote database URLs, ambiguous discovery, ambient `COMPOSE_*` overrides, or multi-consumer targets are rejected before mutation.',

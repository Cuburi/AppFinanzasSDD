...
Before promising or producing images, inspect the active harness for a real image-generation tool.

- If one is available, use it and retain the per-section image workflow below.
- If none is available, do not claim to render images and do not fail. Instead, produce a complete, ordered, copy-ready prompt pack with one prompt per required section or frame.
- Each fallback prompt must include the section name, expected aspect ratio or viewport, shared visual-continuity constraints, and section-specific composition and content.
- Include brief usage instructions for submitting the prompts sequentially to an external image generator and preserving continuity between outputs.
- The fallback prepares prompts and mockups only; it must not modify application source.

When image-generation capability is absent, this fallback explicitly overrides every later unconditional instruction to generate, render, or return images. Section counts, separation, order, and art-direction guidance still apply to the prompt pack.

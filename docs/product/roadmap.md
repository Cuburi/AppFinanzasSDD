...
| Current canonical source commit/version | Working-tree roadmap refresh after RM-012 publication via PR #194 (`d62775b`); this refresh is pending normal Git review and publication. |
...
|---|---|---|---|---|---|
| RM-027 | Establish a coherent UI direction through a screen-by-screen audit before redesign work. | Audit, product hierarchy, navigation/layout direction, critical-flow friction, state inventory, and component consolidation candidates only; no full redesign, visual rebuild, or absorption of concrete UX work. | `unfinished` / `Now` (1) | The product's primary promise is understanding monthly availability within 30 seconds and recording expenses with low friction; current maintainer feedback indicates the UI does not yet earn confidence. The audit must precede broad interaction changes so UI work stays bounded and evidence-led. | No completion claim. `C-30`; `maintainer:2026-08-23,2026-09-20`; `PRODUCT.md`; active roadmap review; GitHub issue #195. |

## Next — correctness, current experience, and reporting closure

| ID | Outcome | Scope / non-goals | Status / horizon | Dependencies and rationale | Completion evidence / provenance |
|---|---|---|---|---|---|
| RM-021 | Make money entry formatting, zero/placeholder behavior, and stepper behavior consistent. | No prescribed shared component, locale, validation policy, or visual redesign. | `unfinished` / `Next` (1) | Requires a money-entry surface inventory and focused evidence that validation and decimal semantics remain preserved. It should follow the UI audit because money entry is part of the daily critical path. | No completion claim. `C-21`; `maintainer:2026-08-23`. |
| RM-024 | Protect users from accidental destructive loss where the risk warrants confirmation. | No global confirmation pattern; action inventory, thresholds, language, and undo alternatives remain open. | `unfinished` / `Next` (2) | Depends on product-specific action inventory and consequences. It should use the audit output to avoid inconsistent confirmation patterns. | No completion claim. `C-27`; `maintainer:2026-08-23`. |
| RM-023 | Restrict subcategory choices to the selected category. | Excludes taxonomy management and grouped presentation; edit, migration, and empty states remain open. | `unfinished` / `Next` (3) | Depends on authoritative relationships and preservation of historic records; pairs with category/subcategory overload reduction. | No completion claim. `C-26`; `maintainer:2026-08-23`. |
| RM-006 | Reduce category/subcategory and income information overload through grouped organization. | Does not choose wrappers, defaults, persistence, or selection/filtering behavior. | `unfinished` / `Next` (4) | Needs a presentation-surface inventory; remains distinct from RM-022/RM-023. | No completion claim. `C-22`, merged `C-23`; `maintainer:2026-08-23`. |
| RM-009 | Preserve browseable, addressable credit-card statement history for reconciliation and payment linkage. | Stable identity/history policy; a rolling summary is not an audit trail. | `unfinished` / `Next` (5) | Must define later-edit reconciliation and focused identity/history design; precedes RM-010. | Current endpoint is a rolling snapshot. `C-10`; `docs/product/technical-debt.md`; `engram:#1198`. |
| RM-010 | Track full or partial payment of closed statements without double-counting consumption. | Statement-level payment semantics only; no inferred payments from balance changes. | `unfinished` / `Next` (6) | Depends on RM-009 stable history and financial-correctness rules. | No implementation found. `C-09`; `docs/product/technical-debt.md`. |
| RM-013 | Establish accessibility, responsiveness, coverage, lint, and assertion-quality evidence. | Safeguard group only; later work may split it. | `quality/debt` / `Next` (7) | Supports every user-facing initiative; relevant evidence gaps and a focused plan must resolve before broad UI implementation. | Selected browser evidence exists; broader quality evidence is open. `C-13`; `engram:#11`. |
| RM-019 | Provide a bounded comparison/reporting capability across distinct months. | Excludes claims that active-month reporting is incomplete and excludes PDF export. | `unfinished` / `Next` (8) | Depends on RM-001 and a maintainer-defined comparison outcome; follows the immediate integrity and primary-experience sequence. | Current reports are active-month only. `C-19`; product plans and current report paths. |
...
| RM-026 | Record an expense without category or subcategory while preserving truthful monthly totals and an `Uncategorized` display. | Category/subcategory are optional; later categorization is optional; an expense may remain uncategorized indefinitely, including after close. Budget and broader analytics treatment remain separate design work. | `shipped` / `Shipped` (5) | Depends on RM-001 and maintainer-accepted accounting semantics. It removed a daily recording barrier before RM-012 retry protection. | Implemented and merged through PR #190. `C-29`; `maintainer:2026-08-23,2026-08-25`; feature commit `2296310`; merge commit `a5cd000`; archived SDD change `uncategorized-expense-recording`. |
| RM-012 | Prevent retried financial CREATE requests from producing duplicate movements. | Request-level CREATE protection only; excludes transactions and blanket PATCH/DELETE idempotency. | `shipped` / `Shipped` (6) | A bounded integrity safeguard for safe daily use; protects existing and future money-moving workflows after RM-026 removed the daily uncategorized-entry barrier. No hard technical dependency on RM-026 has been established. | Implemented and merged through PR #194. `C-12`; `maintainer:2026-08-30`; GitHub issue #193; feature commit `f954075`; merge commit `d62775b`; ODD task `odd/tasks/rm012-idempotent-financial-creates.md`. |
...
| 2026-09-20 | Refreshed the roadmap after RM-026 delivery: RM-026 moved to `shipped` / `Shipped`, RM-012 became `Now` sequence 1, and RM-027 moved from `Explore` to `Now` sequence 2 as a bounded UI audit/direction step before broad redesign or interaction work. | PR #190 merge `a5cd000`; feature commit `2296310`; PR #192 merge `89aee71`; maintainer UI-priority review. |
